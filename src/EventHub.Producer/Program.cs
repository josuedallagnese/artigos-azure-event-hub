    ...
    class Program
    {
        public static async Task Main()
        {
            ...
            Assembly.GetExecutingAssembly()
                .DefinedTypes
                .Where(x => x.GetInterfaces().Contains(typeof(IProducer)))
                .ToList()
                .ForEach(type => services.Add(new ServiceDescriptor(typeof(IProducer), type, ServiceLifetime.Transient)));

            using var scope = services.BuildServiceProvider().CreateScope();

            var jobs = scope.ServiceProvider.GetServices<IProducer>().ToList();
